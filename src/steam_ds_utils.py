...
def data_manager(appid, data_type):
    ...
    *** possible bug here: if a string of an appid is passed, the data
    ...
    2022-05-21 todo:
        ...
        - does this violate 2sec web scrape timing?
    '''
    
    '''
    2023-06-19 todo:
        just download raw json file for each app
        THEN build the stuff you want later
    '''
...
# score a game! 
game_tag_scorer('251570', user1_fingerprint[['name','attr_prop_sum']])

# score a list of games!
candidates_list = [1144200,1149460,314650,393380]
candidates_data = []
...
def get_playtime_percentiles_from_steam(appid,results):
    ...
    if new cursor but no new reviews, kill the loop
    
    maybe just input how many PAGES of data you want
    about 100 results per page
    default at 1 page since the cursor pagination is sucky
    '''
    ...
    percentiles_dict = {
        ...
        "playtimes":list(final_percentiles['playtime_forever'])
        }
...
def get_playtime_percentiles_from_steam_2(appid, pages=1):
    '''
    page-based version, defaults at 1 page, about 100 results per page
    howlongis.io does about 200 reviews per appid i think    
    '''
    # appid = 2420510 
    # pages = 3
    ...
    while pages_processed < pages:
        print('processing appid {}, cursor {}, page {}/{}'.format(appid, start_cursor, pages_processed, pages))
        s = requests.Session()
        reviews_response = s.get('https://store.steampowered.com/appreviews/{}?json=1&filter=all&day_range=365&num_per_page=100&cursor={}'.format(appid,start_cursor))
        ...
        try:
            if json_load['cursor'] == start_cursor:
                print("duplicate cursor")
                # break
            else:
                start_cursor = json_load['cursor']
        except:
            print("cursor error")
            break
    ...
    playtimes = list(reviews_data['playtime_forever'].quantile([.1, .25, .5, .75, .9])/60)
    playtimes_rounded = [ '%.2f' % elem for elem in playtimes ]

    percentiles_dict = {
        "appid":appid,
        "n_reviews": len(reviews_data),
        "playtimes":playtimes_rounded
        }

    return percentiles_dict
    
# test_data = get_playtime_percentiles_from_steam_2(1958340)
...
'''
two app smoke test for sseagal

244210 - assetto corsa - high engagement all tags
341190 - HM2 digital comic book - low engagement few tags

current setup will auto-bias towards comic book. how to balance?
'''

# sseagal_test = user1[user1['appid'].isin(['341190','244210']) ]
# sseagal_fingerprint = compute_profile_fingerprint(sseagal_test)
...
balance_score	
=sqrt(rank(J14,J:J,0)^2+ rank(I14,I:I,0)^2)

blance_score2
=sqrt(rank(J14,J:J,0)^2+ rank(I14,I:I,0)^2 + rank(N14,N:N,0)^2)
...
deckbuilder?
=iferror(if(search("Deck",H14)>0,1,0),0)
...
owned_by_scott
-- join with owned appids from AI52487963 library?
'''
...
'''
appids_list = pd.read_clipboard()
appids_list_only = list(appids_list['appid'])

appids_list_only = appset
appids_list_only = ['262060', '1940340']

playtime_results = pd.DataFrame()

for i in range(0,len(appids_list_only)):
    # i = appids_list_only[0]
    # i=0
    
    print("{}/{}".format(i, len(appids_list_only)))
    
    try:
        game_playtime = get_playtime_percentiles_from_steam_2(appids_list_only[i])
    except:
...
seagal_games = pd.DataFrame(get_users_games("443EE200F8889091D47FA9F6AF615A4B", 76561197969025704))
full_data_read = pd.read_csv("F:\\docs\\dev\\python\\sds-2022-05-20\\steam_data_science_copy\\2023-03-27\\full_data.csv")

full_data_owned = pd.merge(full_data_read, seagal_games[['appid','name']], how="left", on="appid")

full_data_owned.to_csv("F:\\docs\\dev\\python\\sds-2022-05-20\\steam_data_science_copy\\2023-03-27\\full_data_owned.csv")
